...
function AppContent() {
  ...
  if (isAdminView) {
    ...
    return (
      ...
      >
        ...
        {currentPage === 'admin-media' && <AdminMediaLibrary />}
        {currentPage === 'admin-services' && <AdminServices />}
        {currentPage === 'admin-testimonials' && <AdminTestimonials />}
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between selection:bg-amber-400 selection:text-black">
      ...
      {/* Global Client-Facing WhatsApp Floating Button */}
      {currentPage !== 'admin-login' && (
        <WhatsAppButton
          currentPage={currentPage}
          currentService={routeParams.service}
        />
      )}
    </div>
  );
