...
export const StudioProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [design, setDesign] = useState<DesignSettings>(() => {
    try {
      const cached = localStorage.getItem('studio_design_settings');
      if (cached) return { ...DEFAULT_DESIGN, ...JSON.parse(cached) };
    } catch (e) {}
  ...
  const [content, setContent] = useState<WebsiteContent>(() => {
    try {
      const cached = localStorage.getItem('studio_content_settings');
  ...
  const updateDesign = async (newDesign: Partial<DesignSettings>) => {
    ...
    setDesign(updated);
    try {
      localStorage.setItem('studio_design_settings', JSON.stringify(updated));
    } catch (e) {}
  ...
  const updateSiteSettings = async (newSettings: Partial<SiteSettings>) => {
    ...
    try {
      localStorage.setItem('studio_site_settings', JSON.stringify(updated));
    } catch (e) {}
  ...
  const updateContent = async (newContent: Partial<WebsiteContent>) => {
    ...
    setContent(updated);
    try {
      localStorage.setItem('studio_content_settings', JSON.stringify(updated));
    } catch (e) {}
  ...
  const resetDesignDefaults = async () => {
    ...
    try {
      localStorage.removeItem('studio_design_settings');
    } catch (e) {}
