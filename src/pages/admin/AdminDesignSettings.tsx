...
export const AdminDesignSettings: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSaved(true);
      showSuccess('Design settings updated successfully.');
      setTimeout(() => setSaved(false), 3000);
    } catch (err: any) {
      showError(err.message || 'Failed to save design settings.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      showSuccess('Design reset to default styling.');
    } catch (err: any) {
      showError(err.message || 'Failed to reset design.');
    }
  };
