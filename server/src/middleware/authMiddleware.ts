...
export function getAdminEmails(): string[] {
  const envEmail = process.env.ADMIN_EMAIL || 'nilora23x@gmail.com';
  const additional = (process.env.ADDITIONAL_ADMIN_EMAILS || '')
    .split(',')
    .map((e) => e.trim().toLowerCase())
    .filter(Boolean);

  return Array.from(new Set([envEmail.toLowerCase(), ...additional]));
}
...
async function verifyFirebaseIdToken(token: string): Promise<{ valid: boolean; email?: string; uid?: string; error?: string }> {
  ...
  const validAudiences = [PROJECT_ID, 'oceanic-carrier-1dtd', 'oceanic-carrier-1dtd0'];
