...
export async function getPortfolioItems(): Promise<ServerPortfolioItem[]> {
  try {
    ...
    // Collection is empty: return initial defaults directly without write stream flood
    return INITIAL_PORTFOLIO.map((p, i) => ({ ...p, id: p.id || `default_${i}`, order: i + 1 }));
  } catch (err) {
    console.warn('[portfolioService] Fallback to initial portfolio items:', err);
    return INITIAL_PORTFOLIO.map((p, i) => ({ ...p, id: p.id || `default_${i}`, order: i + 1 }));
  }
}
...
export async function updateMediaAsset(id: string, updates: Partial<MediaAsset>): Promise<MediaAsset | null> {
  try {
    ...
    console.error('[portfolioService] Error updating media asset:', err);
    return null;
  }
}
